...
| **Status** | Approved for Engineering — **clean slate 2026-09-15: sistem dibangun dari ulang, seluruh fase backend (B0–B12) ⬜ belum dimulai**; frontend F1–F4 ⬜ (gate: B0–B6) |
...
| TK103 | 5013 | `TK103_TCP_PORT` | ✅ B9 (login+handshake BP00/BP05, posisi, odometer, 7 perintah downlink) |
...
| Suntech (ST215/ST240/ST340) | 5017 | `SUNTECH_TCP_PORT` | ✅ B9 (teks universal klasik: posisi+identitas; varian biner/per-model belum) |
| H02 / H08 | 5010 | `H02_TCP_PORT` | ✅ B9 (teks V0/HTBT/V3; biner belum) |
| Totem | 5005 | `TOTEM_TCP_PORT` | ✅ B9 (PATTERN_1 GPRMC + PATTERN_2 pipe-delimited) |
...
| Castel (SC/CC/MPIP) | 5019 | `CASTEL_TCP_PORT` | 🟡 B9 (framing + balasan login/heartbeat; posisi GPS opt-in `CASTEL_GPS_DECODE`) |
...
| **B8** | Advanced Fleet Features (downlink, driver behavior, maintenance) | ✅ Selesai 2026-09-24 + audit lanjutan (downlink `DYD#` end-to-end + ACK perangkat, encoder GT06 **& TK103 (7 perintah)**, konsumsi **JetStream durable** + 2 guard, E2E `make e2e-commands` 5/5; driver score **ternormalisasi per 100 km** (migrasi 025) + maintenance reminder; gap: encoder keluarga lain, atribusi skor ke driver) — `docs/B8-B10-VERIFICATION.md` |
| **B9** | Protocol Expansion (Meiligao, Xexun, Suntech, H02, Totem, GT02, Navigil, Castel; validasi TK103) — port & decoding per referensi Traccar | 🟡 Sebagian 2026-09-25 (decoder pluggable + 9 keluarga: ingest penuh TK103/Meiligao/Xexun/H02 teks/Totem P1+P2/GT02/Navigil MSG 8+18/Suntech teks klasik, Castel framing+respons+posisi opt-in; E2E nyata Xexun/Navigil/Suntech/Totem/Meiligao/Castel) — `docs/B8-B10-VERIFICATION.md` |
...
| **B11** | **Governance & Data Lifecycle** — audit trail wajib `tm_audit_logs` (§9.4), soft delete global + endpoint restore (§6.0.1), auto-create admin tenant password `Admin@123` (FR-5.5), migrasi DB otomatis di Coolify (§14.5), dukungan protokol universal (Module 1c) | ✅ Selesai 2026-09-26 (audit middleware untuk semua mutasi + penolakan `ACCESS_DENIED`, `GET /audit-logs` Admin, registry protokol universal `internal/protocol` + `tm_protocols` + `tm_vehicles.protocol`/`tm_vehicle_imei_map.protocol`, brand tak dikenal → 400; migrasi master `021`–`024`/company `026`–`027` diuji apply+ROLLBACK nyata) — `docs/B11-B12-VERIFICATION.md` |
| **B12** | **Enterprise & Industry Modules** (acuan `docs/FRONTEND.md`, §5.10 Module 9) — drivers, groups, personel/kartu RFID/log akses, assets, maintenance, safety score & incidents, laporan/analitik lanjutan, organization, integrations (API/Webhook), share lokasi publik, heatmap; modul industry-specific (rental, transport, logistics, sales, field service, patrol, project site) bertahap; Personal/B2C (§4.2, FR-9.2); registry module & menu **master** (`tm_modules`/`tm_menus`, seed FRONTEND.md) + role menu access **per-tenant** (`tm_role_menu_access`, §6.2) | 🟡 Core selesai 2026-09-26 (`GET /access/menu` per role + matriks role→menu, lisensi modul `tm_company_modules` + gating industri, CRUD tabel-driven drivers/groups+members/personnel/cards/access-logs/assets/incidents/organizations/maintenance dengan soft delete+restore+audit, integrations API key/webhook, share publik `GET /api/v1/share/{token}`, heatmap+rebuild, reports trips/violations, safety scores; **sisa**: halaman per-sub-modul industri, Personal/B2C, export/scheduled report, settings tenant, E2E harness) — `docs/B11-B12-VERIFICATION.md` |
...
2. **Keterbatasan protokol perangkat** — dari 200+ protokol Traccar, kini aktif: GT06, Teltonika,
   + keluarga B9 (TK103 login/handshake/posisi/odometer + 7 perintah downlink, Meiligao lintas-id
   + Luhn, Xexun, H02 teks, Totem P1+P2, GT02 penuh, Navigil MSG 8/18, Suntech teks universal
   klasik; Castel framing+identitas+balasan login/heartbeat + posisi GPS opt-in
   `CASTEL_GPS_DECODE`). Payload yang belum terdokumentasi/saling bertentangan di
   `docs/docs-device/traccar-reference/` TIDAK ditebak — dihitung
   `ingestion_unsupported_frames_total` (detail: `docs/B8-B10-VERIFICATION.md` §4/§2.8).
...
| # | Fitur | Estimasi | Fase |
|---|---|---|---|
| 1 | **Downlink / Remote Commands** — connection registry, engine cut-off (`DYD#`), interval change, reboot | 5 d | ✅ B8 (2026-09-24; encoder GT06 + **TK103 7 perintah** (`AV010`/`AV011`/`AT00`/`AP00`/`AP07`/`AR00<hex>`/`AR0000000000`), `td_device_commands` audited, konsumsi JetStream durable; gap: encoder keluarga lain) |
| 2 | **Odometer & Engine Hours** | 2 d | B7.1 ✅ (2026-09-24) |
| 3 | **Trip & Stop Detection** | 5 d | B7.2 ✅ (2026-09-24) |
| 4 | **Driver Behavior Analysis** (harsh accel/braking/cornering — Teltonika IO 253/254, Concox alarm 0x09/0x0A) | 3 d | ✅ B8 (2026-09-24; `td_driver_events` + `th_driver_scores` **ternormalisasi per 100 km** (migrasi 025) + alert; sisa gap: atribusi `driver_id`) |
