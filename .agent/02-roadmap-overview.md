...
>
> **STATUS 2026-09-25 (penutupan GAP B8/B9 — gelombang 2):** seluruh gap yang dapat
> diverifikasi sudah ditutup — **Totem PATTERN_2** + ekstraksi IMEI dua tata letak,
> **handshake TK103 `BP00/BP05`** + odometer `L<hex>` + terminator `)`,
> **union id Meiligao** (frame device nyata 0x9999) + **Luhn** untuk id 14 digit,
> **posisi Castel opt-in** `CASTEL_GPS_DECODE=off|on|on-swapped` (skala/field
> terverifikasi dari patch upstream resmi `/3600000`, `cms→km/h`, `/10`),
> **skor mengemudi per 100 km** (migrasi `025`), **payload Suntech teks klasik**,
> dan **re-point IMEI tervalidasi via PATCH** (`validate.IMEI` + `IMEIExists` +
> pindah peta allowlist). Bukti live: `docs/B8-B10-VERIFICATION.md` §2.4–§2.7.
> Yang masih memblokir (rujukan saling bertentangan / tidak dapat diambil utuh):
> H02 biner, Meiligao OBD/DTC/RFID, Navigil MSG 13/15, sisa matriks TK103, atribusi
> skor ke `driver_id` (B12) — semuanya dihitung/tercatat, bukan ditebak (§2.8/§4).
> **payload posisi Suntech** (teks universal klasik: header;id;versi;YYYYMMDD;HH:MM:SS;
> [cell;]lat;lon;speed;course, lat/lon bertanda, speed km/h; E2E `:9017` → DB + Redis),
> **matriks perintah TK103** (`device_version`/`AP07` + `position_stop`/`AR0000000000`
> masuk model, whitelist API, encoder, migrasi `024` + CHECK DB, `jsadmin`), dan
> **skor mengemudi ternormalisasi per jarak** (migrasi `025`: `distance_km`/
> `events_per_100km`/`score_by_counts`, bucket A..E, fallback jumlah di bawah
> `DRIVER_SCORE_MIN_DISTANCE_KM=5`; E2E alarm `0x26/0x30` + trip 200 km → 5 poin/100 km
> → 100/A). Gap yang masih terbuka dicatat di `docs/B8-B10-VERIFICATION.md` §4
> (skala GPS Castel, varian biner/per-model Suntech, Meiligao OBD/RFID, H02 biner,
> Totem PATTERN_2, sisa matriks TK103, atribusi skor ke `driver_id`, validasi IMEI PATCH).
> ACK Navigil 24 byte, payload Navigil MSG 8/18, peta identitas `NAVIGIL_DEVICE_MAP`
> (+metrik `ingestion_unmapped_devices_total`), Castel `Length`=seluruh frame + balasan
> login/heartbeat `0x9001/0x9003` (+metrik `ingestion_castel_crc_errors_total`), downlink
> **JetStream durable** (`internal.NATSClient.QueueSubscribeDurable`, `MaxDeliver=5`),
> bug SQL audit command (`42P08` → timestamp dihitung di Go), encoder downlink **TK103**,
> dan **E2E downlink baru `scripts/e2e-commands.sh` (`make e2e-commands`) 5/5 PASS**
> (simulator GT06: login → `0x80 DYD#` → balasan `0x21` → `td_device_commands(acked)`,
> plus `offline` dan pengiriman durable setelah restart). Gap tersisa dicatat di
> `docs/B8-B10-VERIFICATION.md` §4 (payload posisi Suntech/Castel menunggu dokumen vendor,
> skor mengemudi belum per-jarak, encoder keluarga selain GT06/TK103).
