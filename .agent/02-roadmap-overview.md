...
>
> **STATUS 2026-09-25 (penutupan GAP B8/B9):** tiga gap yang tersisa ditutup —
...
| **B9** | Protocol Expansion (Meiligao, Xexun, Suntech, H02, Totem, GT02, Navigil, Castel; validasi TK103) — port & decoding per referensi Traccar | `ingestion-tcp` | 🟡 Sebagian 2026-09-24 (+ audit lanjutan & GAP) — arsitektur decoder pluggable (`Decoder`+registry, 11 listener), **ingest penuh**: TK103/Meiligao/Xexun/H02(teks)/Totem(P1)/GT02/**Navigil (MSG 8+18)**/**Suntech (teks universal klasik)**; **framing+identitas+respons**: **Castel (framing `Length` total-frame + balasan login/heartbeat)**; payload posisi Castel GPS masih `ingestion_unsupported_frames_total`. E2E nyata: Xexun `:9004`, Navigil `:9012`, Suntech `:9017` → `th_telemetry_logs` + Redis live state. Bukti: §2 |
