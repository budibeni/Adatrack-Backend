...
>
> **STATUS 2026-09-24 (audit lanjutan B8–B10 → perbaikan gap):** 9 temuan audit ditutup —
...
| **B8** | Advanced Fleet Features (downlink/remote commands `DYD#`, driver behavior, maintenance scheduling) | `ingestion-tcp`, `worker-alert`, `api-vehicle` (+ migrasi company `021`–`023`) | ✅ Selesai 2026-09-24 (+ audit lanjutan) — downlink end-to-end (registry→encoder GT06 `0x80` **dan TK103** `AV010/AV011/AT00/AP00/AR00`→`td_device_commands`→ACK `0x21`), konsumsi **JetStream durable** (command tidak hilang saat restart), driver behaviour (`td_driver_events`/`th_driver_scores`, alert `driver_event`), maintenance reminder (`tm_maintenance_schedules`/`td_maintenance_logs`, alert `maintenance_due`), REST `POST|GET /vehicles/{id}/commands`, E2E `scripts/e2e-commands.sh` 5/5. Gap: encoder keluarga lain `failed: unsupported`, skor belum dinormalisasi per jarak. Bukti: `docs/B8-B10-VERIFICATION.md` §1 |
| **B9** | Protocol Expansion (Meiligao, Xexun, Suntech, H02, Totem, GT02, Navigil, Castel; validasi TK103) — port & decoding per referensi Traccar | `ingestion-tcp` | 🟡 Sebagian 2026-09-24 (+ audit lanjutan) — arsitektur decoder pluggable (`Decoder`+registry, 11 listener), **ingest penuh**: TK103/Meiligao/Xexun/H02(teks)/Totem(P1)/GT02/**Navigil (MSG 8+18, peta `NAVIGIL_DEVICE_MAP`)**; **framing+identitas+respons**: Suntech/**Castel (framing `Length` total-frame diperbaiki, balasan login/heartbeat 0x9001/0x9003)** dengan payload posisi masih `ingestion_unsupported_frames_total`. E2E nyata: Xexun `:9004` → `th_telemetry_logs` + Redis live state. Bukti: §2 |
