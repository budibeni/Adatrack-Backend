...
> Panduan utama untuk semua kerja di project `adatrack_gps`. Selalu dibaca sebelum mulai implementasi.
...
4. **Satu layanan satu module Go.**
   - Module pattern: `adatrack_gps/<service-name>` per service (`adatrack_gps/ingestion-tcp`, `adatrack_gps/worker-persistence`, `adatrack_gps/worker-live`, `adatrack_gps/service-websocket`, `adatrack_gps/api-vehicle`, dst.). Ikuti pola ini untuk semua service (clean slate — pola ditetapkan sejak awal pengerjaan).
