...
// ---------------------------------------------------------------------------
// HTTP middleware: CORS, Prometheus metrics, per-user API rate limit.
// ---------------------------------------------------------------------------

// corsMiddleware allows configured origins (PRD §7 CORS_ALLOWED_ORIGINS).
...
// ---------------------------------------------------------------------------
// API rate limiting (GAP #12: 100 req/menit per user)
// ---------------------------------------------------------------------------
...
var apiLimiter *apiRateLimiter // initialized in setupRouter

// apiRateLimitMiddleware enforces per-user API limits on authenticated routes.
func apiRateLimitMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !apiLimiter.allow(key) {
			...
			writeError(c, http.StatusTooManyRequests, "RATE_LIMIT_EXCEEDED",
...
// (CSP, HSTS, X-Frame-Options, etc.) on every response — Phase B4.2 (GAP #12).
