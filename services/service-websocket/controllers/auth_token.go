...
// pola identik api-vehicle agar kontrak auth seragam lintas API.
// ---------------------------------------------------------------------------
...
func issueTokenPair(ctx context.Context, mu models.MasterUser, role string, vehicleIDs []int64) (access string, expSec int64, refresh string, err error) {
	access, expSec, err = signToken(appCfg, mu, role, vehicleIDs)
	if err != nil {
		return "", 0, "", err
	}
	...
	if rerr != nil {
		...
		// Access tetap diterbitkan; refresh kosong = degraded (tidak silent).
...
func authRefreshHandler(c *gin.Context) {
	...
	newRefresh, rerr := tm.RotateRefresh(ctx, req.RefreshToken, appCfg.JWT.RefreshExpiry)
	if rerr != nil {
		if errors.Is(rerr, tokenauth.ErrInvalidToken) {
			writeError(c, http.StatusUnauthorized, "UNAUTHORIZED", "invalid or expired refresh token")
			return
		}
		slog.Error("refresh rotation failed", "error", rerr, "user_id", payload.UserID)
		writeError(c, http.StatusServiceUnavailable, "SERVICE_UNAVAILABLE", "service temporarily unavailable")
		return
	}
	...
	token, expSec, err := signToken(appCfg, mu, payload.Role, payload.VehicleIDs)
	if err != nil {
...
// authLogoutHandler revokes access jti (denylist) dan/atau refresh token.
// Tanpa requireAuth agar klien tetap bisa logout saat access token expired.
func authLogoutHandler(c *gin.Context) {
	...
	if tok := extractToken(c); tok != "" {
		if claims, err := parseToken(appCfg, tok); err == nil && claims.RegisteredClaims.ID != "" {
